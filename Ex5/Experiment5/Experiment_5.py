    ...
    if q_dialog.exec() == QDialog.Accepted:  # 提取用户交互的参数
        np_kernel = np.array(
            [[int(dlg.lineEdit1.text()), int(dlg.lineEdit2.text()), int(dlg.lineEdit3.text()), int(dlg.lineEdit4.text()), int(dlg.lineEdit5.text())],
             [int(dlg.lineEdit6.text()), int(dlg.lineEdit7.text()), int(
                 dlg.lineEdit8.text()), int(dlg.lineEdit9.text()), int(dlg.lineEdit10.text())],
             [int(dlg.lineEdit11.text()), int(dlg.lineEdit12.text()), int(
                 dlg.lineEdit13.text()), int(dlg.lineEdit14.text()), int(dlg.lineEdit15.text())],
             [int(dlg.lineEdit16.text()), int(dlg.lineEdit17.text()), int(
                 dlg.lineEdit18.text()), int(dlg.lineEdit19.text()), int(dlg.lineEdit20.text())],
             [int(dlg.lineEdit21.text()), int(dlg.lineEdit22.text()), int(
                 dlg.lineEdit23.text()), int(dlg.lineEdit24.text()), int(dlg.lineEdit25.text())]
             ])
...
def erosion_binary(img, np_kernel):  # finished
    ...
    *注意，传入的图像必须为二值图像, kernel为结构形状, 函数：XΘS={x|S+x∈X}"""
    ...
    # new_img = cv.erode(img, np.uint8(np_kernel))
...
def dilation_binary(img, np_kernel):  # finished
    ...
    *注意，传入的图像必须为二值图像, kernel为结构形状, 这里用的是腐蚀函数的对偶运算：
    ...
    # new_img = cv.dilate(img, np.uint8(np_kernel))
...
def fast_thin(img, deal_Type):
    ...
    binary[binary==255] = 1
    new_img0 = morphology.skeletonize(binary)
    new_img = new_img0.astype(np.uint8)*255
    time2 = time.time()  # 程序计时结束
...
def erosion_gray(img, np_kernel):
    ...
    for i in range(2, rows - 2):
        for j in range(2, cols - 2):
            img_kernel = np.array(img[i - 2:i + 3, j - 2:j + 3])
            img_kernel = img_kernel + np_kernel
            min = np.min(img_kernel)
            if min > 255:
                new_img[i, j] = 255
            new_img[i, j] = min
...
def morphy_gray_edge(img, np_kernel):
    ...
    for i in range(rows):
        for j in range(cols):
            new_img[i][j] = abs(dil_img[i][j]-ero_img[i][j])
    time2 = time.time()  # 程序计时结束
